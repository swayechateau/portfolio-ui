...
import { langs, lang } from './state';
import LanguageSwitcher from './LanguageSwitcher';
export interface INav {
  yAxis: number;
}
function DesktopNav({ yAxis = 40 }: INav) {
  const [show, setShow] = useState(false);
  const [test, setLang] = useState(lang);
  ...
  return (
    <nav
      className={`fixed w-full top-0 min-h-max h-16 z-50 transition-all ease-in hidden md:flex print:none ${
        show
    ...
    >
      ...
      <div className="flex items-center justify-center space-x-3 p-1">
        <LanguageSwitcher absolute={false} show={show} current={test} langs={langs} onSwitch={setLang} />
      </div>
    </nav>
  );
}
